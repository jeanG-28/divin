...
module.exports = async function handler(req, res) {
  ...
  if (!process.env.STRIPE_SECRET_KEY || !process.env.STRIPE_WEBHOOK_SECRET_PAIEMENT) { res.status(503).end(); return; }
  ...
  let event;
  try {
    event = stripe.webhooks.constructEvent(corpsBrut, signature, process.env.STRIPE_WEBHOOK_SECRET_PAIEMENT);
  } catch (e) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (userId && process.env.SUPABASE_SERVICE_ROLE_KEY) {
      ...
      if (!sbRes.ok) { console.error('Echec mise a jour Supabase (paiement):', sbRes.status, sbBody); }
      else { console.log('Mise a jour Supabase OK (paiement):', sbBody); }
    } else {
